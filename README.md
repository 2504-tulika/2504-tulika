# 👋 Hi, I’m Tulika Lunkad  
💡 *Data Science & Analytics Engineer | Machine Learning | AI & Visualization Enthusiast*  

I’m passionate about transforming complex datasets into **actionable insights** through clean code, visualization, and intelligent ML systems.  
Currently exploring **LLM-powered automation**, **data analytics**, and **AI for problem-solving** — blending technical depth with real-world impact.

---

## 🧠 About Me
- 🎓 **B.Tech in Computer Science (Data Science)** – Bhilai Institute of Technology, Durg (CGPA: 9.55)  
- 💼 **Data Science (ML) Intern – IIT Guwahati (May–Aug 2025)**  
  Worked on **human activity anomaly detection** using skeletal kinematics and deep learning (PyTorch, PoseFlow, AlphaPose).  
- 🧮 Skilled in **Python, MySQL, Power BI, Tableau**, and **LangChain** for data analysis, visualization, and AI integration.  
- 🧠 Focus Areas: Machine Learning, NLP, Anomaly Detection, and Data-driven AI Applications.

---

## 🚀 Featured Projects
- Check my GitHub profile for the same and connect with me for the same.

## 💼 Experience

### 🧠 Data Science Research Intern – IIT Guwahati  
- Implemented **COSKAD** (Contracting Skeletal Kinematics for Anomaly Detection) model on HR-UBnormal dataset.  
- Adapted pose estimation and graph encoding for CPU-based inference.  
- Achieved near-benchmark performance using **PyTorch Lightning, NumPy, and Matplotlib**.

### 💻 Web Development Intern – The Sparks Foundation  
- Developed a **Basic Banking System** using PHP, MySQL, and JavaScript.  
- Designed normalized relational schemas for transaction tracking and ensured data integrity.

---

## 🧰 Tech Stack
- **Programming and Tools:** Python, SQL, Scikit-learn, TensorFlow, Keras, LangChain, Power BI, Tableau
- **Machine Learning & AI:** Classification, Regression, Clustering, NLP, LLM Integration, Prompt Engineering
- **Data Science & Analytics:** Data Wrangling, Feature Engineering, EDA, Statistical Modeling, Predictive Analytics
- **Database & Cloud:** MySQL, MongoDB, Google BigQuery, Flask APIs

---

## 🌱 Currently Exploring
- Large Language Models (LLMs) and Retrieval-Augmented Generation (RAG)  
- Scalable AI-driven analytics dashboards  
- Research in video-based anomaly detection  

---

## 📫 Let’s Connect
💼 [LinkedIn](https://www.linkedin.com/in/tulika-lunkad-9a6536255)  
📧 [tulika2504@gmail.com](mailto:tulika2504@gmail.com)  

---

